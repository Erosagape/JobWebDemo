...
function SetGridAdvance(p, g, d, q, ev) {
    $(g).DataTable({
        ajax: {
            url: p + 'Adv/GetAdvanceGrid' + q, //web service ที่จะ call ไปดึงข้อมูลมา
            dataSrc: 'adv.data[0].Table'
        },
        selected: true, //ให้สามารถเลือกแถวได้
        columns: [ //กำหนด property ของ header column
            { data: null, title: "#" },
...
function SetGridClearing(p, g, d, q, ev) {
    $(g).DataTable({
        ajax: {
            url: p + 'clr/GetClearingGrid' + q, //web service ที่จะ call ไปดึงข้อมูลมา
            dataSrc: 'clr.data[0].Table'
        },
        selected: true, //ให้สามารถเลือกแถวได้
        columns: [ //กำหนด property ของ header column
            { data: null, title: "#" },
